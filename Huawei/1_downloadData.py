...
def get_data(code):
    ...
    result.to_csv(str("data/datas/"+code+"_data.csv"), index=False,)
    # print(result)
    return None
...
if __name__ == "__main__":
      #获取沪深300股票代码
    save_path = "data/hs300_stocks.csv"
    get_code(save_path)

      #获取股票价格数据：每支股票2010年至今信息
    with open("data/hs300_stocks.csv", "r",encoding='utf-8') as csvfile:
        reader = csv.reader(csvfile)
        rows = [row for row in reader]
    ...
    print(row_count)
      ...
      #获取筛选后数据
    files_path = "data/datas/"
    save_path = "data/filtered_data.csv"
    loss_path = "data/loss_data.csv"
    if (os.path.exists(save_path)):
        os.remove(save_path)
